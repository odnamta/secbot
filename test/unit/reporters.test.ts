...
describe('Bounty markdown reporter', () => {
  ...
  it('includes code example in fenced code block', () => {
    ...
    const md = readFileSync(path, 'utf-8');
    expect(md).toContain('**Additional Evidence:**');
    expect(md).toContain('```');
  ...
  it('sorts findings by severity (critical first) and excludes info', () => {
    const path = tmpPath('md');
    const findings = [
      ...
      makeInterpretedFinding({ title: 'Low Header', severity: 'low' }),
    ];
    writeBountyReport(makeScanResult(findings), path);
    ...
    const lowPos = md.indexOf('[LOW] Low Header');
    // Info findings are excluded from bounty reports
    expect(md).not.toContain('[INFO] Info Disclosure');
    expect(criticalPos).toBeLessThan(mediumPos);
    expect(mediumPos).toBeLessThan(lowPos);
  });
