...
export async function detectFramework(page: Page): Promise<FrameworkInfo | null> {
  try {
    const result = await page.evaluate(() => {
      ...
      // ── Angular ──
      // Strategy 1: ng-version attribute (Angular 2-17)
      ...
      // Strategy 2: getAllAngularRootElements (debug API)
      ...
      // Strategy 3: window.ng global (Angular debug utilities — modern Angular v14+)
      {
        const ng = (w as Record<string, unknown>).ng;
        if (ng && typeof ng === 'object') {
          const ngObj = ng as Record<string, unknown>;
          if (typeof ngObj.getComponent === 'function' || typeof ngObj.probe === 'function' || typeof ngObj.applyChanges === 'function') {
            evidence.push('window.ng debug utilities found');
            return { name: 'angular' as const, router: 'angular-router' as const, evidence };
          }
        }
      }
      // Strategy 4: Angular view encapsulation attributes (_ngcontent-*, _nghost-*)
      // These are generated by Angular's ViewEncapsulation.Emulated (default) and are very reliable
      {
        const ngContent = doc.querySelector('[class*="_ngcontent-"]') ?? doc.querySelectorAll('*');
        let foundNgAttr = false;
        ...
        for (let i = 0; i < Math.min(allEls.length, 200); i++) {
          ...
          if (foundNgAttr) break;
        }
        if (foundNgAttr) {
          evidence.push('Angular view encapsulation attributes (_ngcontent-/_nghost-) found');
          return { name: 'angular' as const, router: 'angular-router' as const, evidence };
        }
      }
      // Strategy 5: ng-reflect-* attributes (Angular template bindings)
      if (doc.querySelector('[ng-reflect-router-link]') || doc.querySelector('[ng-reflect-ng-class]') || doc.querySelector('[ng-reflect-ng-if]')) {
        evidence.push('ng-reflect-* attributes found');
        return { name: 'angular' as const, router: 'angular-router' as const, evidence };
      }
      // Strategy 6: Angular platform marker via script tags or known Angular patterns
      {
        const scripts = Array.from(doc.querySelectorAll('script[src]'));
        const hasAngularBundle = scripts.some((s) => {
          const src = s.getAttribute('src') || '';
          return /(?:^|\/)(?:main|polyfills|runtime|vendor|chunk-\w+)\.[a-f0-9]+\.js$/i.test(src)
            || src.includes('/angular/')
            || src.includes('@angular');
        });
        // Also check for <app-root> element (Angular's conventional root component)
        const hasAppRoot = !!doc.querySelector('app-root');
        if (hasAppRoot && (hasAngularBundle || scripts.length > 0)) {
          evidence.push('app-root element found');
          if (hasAngularBundle) evidence.push('Angular-style script bundles detected');
          return { name: 'angular' as const, router: 'angular-router' as const, evidence };
        }
      }
...
export async function waitForHydration(
  ...
  try {
    ...
    switch (framework.name) {
      ...
      case 'angular':
        // Wait for Angular to finish bootstrapping — check multiple markers
        await page.waitForFunction(
          () => {
            // Classic ng-version attribute
            if (document.querySelector('[ng-version]')) return true;
            // Modern: view encapsulation attributes
            const allEls = document.querySelectorAll('*');
            for (let i = 0; i < Math.min(allEls.length, 100); i++) {
              const attrs = allEls[i].getAttributeNames();
              for (const attr of attrs) {
                if (attr.startsWith('_ngcontent-') || attr.startsWith('_nghost-')) return true;
              }
            }
            // app-root with rendered children
