...
export const CRLF_PAYLOADS = [
  ...
  // Cache poisoning — inject cache header to poison CDN/proxy
  {
    name: 'cache-poisoning',
    payload: `%0d%0aX-Cache-Poisoned:secbot-cache-test`,
    description: 'Cache poisoning — inject custom cache header',
  },
  // Set-Cookie injection — session fixation via cookie injection
  {
    name: 'set-cookie-injection',
    payload: `%0d%0aSet-Cookie:secbot_test=injected;Path=/`,
    description: 'Set-Cookie injection — session fixation vector',
  },
];

/** All header names/values we inject — any match confirms CRLF */
const INJECTED_MARKERS: Array<{ header: string; value: string }> = [
  { header: CRLF_SENTINEL_HEADER, value: CRLF_SENTINEL_VALUE },
  { header: 'X-Cache-Poisoned', value: 'secbot-cache-test' },
];

/**
 * Check whether a response contains any injected header.
 * Returns the matched header name or null.
 */
export function detectInjectedHeader(
  ...
  for (const [name, value] of Object.entries(responseHeaders)) {
    for (const marker of INJECTED_MARKERS) {
      if (
        name.toLowerCase() === marker.header.toLowerCase() &&
        value.trim() === marker.value
      ) {
        return `${marker.header}: ${marker.value}`;
      }
    }
    // Set-Cookie injection — check if our injected cookie appears
    if (name.toLowerCase() === 'set-cookie' && value.includes('secbot_test=injected')) {
      return `Set-Cookie: secbot_test=injected`;
    }
  }
  return null;
}
    ...
    for (const param of sortedParams) {
      ...
      for (const { url: testUrlHref, payload: crlfPayload } of testCases) {
        ...
        try {
          ...
          const injectedHeaderMatch = detectInjectedHeader(headers);
          ...
          if (injectedHeaderMatch || responseSplit) {
            const isRedirectParam = REDIRECT_PARAM_RE.test(param);
            const isCachePoisoning = crlfPayload.name === 'cache-poisoning';
            const isSessionFixation = crlfPayload.name === 'set-cookie-injection';
            ...
            findings.push({
              id: randomUUID(),
              category: 'crlf-injection',
              severity: isCachePoisoning ? 'critical' : 'high',
              ...
              description: `The parameter "${param}" is vulnerable to CRLF injection. ` +
                ...
                (isCachePoisoning ? 'This can poison CDN/proxy caches, affecting all users.' :
                  isSessionFixation ? 'This can set attacker-controlled cookies for session fixation.' :
              ...
              evidence: [
                ...
                injectedHeaderMatch
                  ? `Injected header found: ${injectedHeaderMatch}`
              ...
              response: {
                status,
                headers: injectedHeaderMatch
                  ? { injected: injectedHeaderMatch }
                  : undefined,
