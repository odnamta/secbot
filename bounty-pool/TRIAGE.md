...
No new scan results since Session 8. Addressed the three unresolved files in `bounty-pool/pending/moneybird/` (created in cycle 18, pre-dating Session 8 triage). All triaged as false positives and archived.

### Moneybird pending/ cleanup — cycle-18 artifacts
...
| `8c0823c1-postmessage-handlers-missing-origin-validation.md` | postMessage missing origin validation | **FP** | www.moneybird.com marketing homepage. 3 postMessage listeners = chat/support widget (Intercom/HubSpot pattern). Known FP: "postMessage from chat widgets — by design." Confidence was "medium" and handler behavior unverifiable via curl. |
| `6d09cce8-dom-based-cross-site-scripting-(xss)-via-url-fragment.md` | DOM XSS via URL fragment | **FP** | Generated by pre-cycle-18 scanner before the DOM XSS FP fix (`6aef881`). The March 22, 2026 re-scan of moneybird returned 0 findings, not reproducing this. Curl cannot execute JavaScript — DOM XSS needs Playwright verification. Page is marketing homepage (www.moneybird.com), not the authenticated accounting app. Even if real, no session data is at risk on the marketing page. Archived as pre-fix FP artifact. |
...
1. **Submit Indeed finding** — CSRF cookie inconsistency (`2026-03-14-indeed-csrf-cookie-SUBMISSION.md` ready). Needs Dio's go-ahead — cookie is JS-set so curl reproduces the header but a browser test should confirm before submission.
